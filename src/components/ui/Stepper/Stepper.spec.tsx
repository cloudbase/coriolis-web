...
describe('Stepper', () => {
  ...
  it('abides by the minimum and maximum', () => {
    ...
    const onChange = (v: number | null) => { value = v }
    const { rerender } = render(<Stepper />)
    const rerenderWithValue = () => {
      rerender(<Stepper value={value === null ? undefined : value} onChange={onChange} minimum={5} maximum={15} />)
    }
  ...
  it.each`
    ...
    ${'7'}     | ${7}
    ${'3'}     | ${5}
  `('dispatches value $dispatchedValue when typing $typedValue', ({ typedValue, dispatchedValue }) => {
    const inputEl = () => TestUtils.selectInput('Stepper__Input')!
    const onChange = jest.fn()
    render(<Stepper value={10} onChange={onChange} minimum={5} maximum={10} />)
