...
describe('Switch', () => {
  it('renders checked and unchecked labels', () => {
    ...
    rerender(<Switch onChange={() => { }} checked={false} uncheckedLabel="Off" />)
  ...
  it('dispatches null when in tristate mode', () => {
    ...
    const { rerender } = render(<Switch onChange={onChange} checked triState />)
  ...
  it('renders with different background if big', () => {
    const { rerender } = render(<Switch onChange={() => { }} checked />)
    const getBackground = () => TestUtils.rgbToHex(window.getComputedStyle(TestUtils.select('Switch__InputBackground')!).background)
    expect(getBackground()).toBe(ThemePalette.primary)

    rerender(<Switch onChange={() => { }} checked={false} uncheckedColor="yellow" />)
