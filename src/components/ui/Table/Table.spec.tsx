...
describe('Table', () => {
  it('renders no items label and no items custom component', () => {
    ...
    rerender(<Table items={[]} header={[]} noItemsComponent={<div className="no-items-component">Zero</div>} />)
    expect(document.querySelector('.no-items-component')).toBeTruthy()
    expect(document.querySelector('.no-items-component')!.textContent).toBe('Zero')
  })
  ...
  it('renders header', () => {
    ...
    expect(TestUtils.selectAll('Table__HeaderData')).toHaveLength(HEADER.length)
    HEADER.forEach((headerItem, i) => {
      expect(TestUtils.selectAll('Table__HeaderData')[i]!.textContent).toBe(headerItem)
    })
    const style = window.getComputedStyle(TestUtils.selectAll('Table__HeaderData')[1]!)
  ...
  it('renders text items', () => {
    ...
    ITEMS.forEach((rowData, rowIndex) => {
      const cellsEl = TestUtils.selectAll('Table__RowData', TestUtils.selectAll('Table__Row-')[rowIndex]!)
  ...
  it('renders items with custom row components', () => {
    const items = [
      [(<div className="row-1-data-1">Row 1 data 1</div>), (<div className="row-1-data-2">Row 1 data 2</div>)],
      [(<div className="row-2-data">Row 2 data</div>)],
    ]
    ...
    expect(TestUtils.selectAll('Table__RowData', rowsEl[0])[0]!.textContent).toBe('Row 1 data 1')
    expect(TestUtils.selectAll('Table__RowData', rowsEl[0])[1]!.textContent).toBe('Row 1 data 2')
    expect(TestUtils.selectAll('Table__RowData', rowsEl[1])[0]!.textContent).toBe('Row 2 data')
  })
})
