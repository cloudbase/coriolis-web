...
describe('StatusPill', () => {
  ...
  `('renders status $status with background $background and label $label', ({ status, background, label }) => {
    render(<StatusPill status={status} />)
    const wrapper = TestUtils.select('StatusPill__Wrapper')!
    expect(wrapper.textContent).toBe(label)
    expect(TestUtils.rgbToHex(window.getComputedStyle(wrapper).background)).toBe(background)
  })
  ...
  it('renders info status with different background colors', () => {
    ...
    expect(TestUtils.rgbToHex(window.getComputedStyle(wrapper()).color)).toBe(ThemePalette.alert)

    rerender(<StatusPill status="INFO" secondary />)
    expect(TestUtils.rgbToHex(window.getComputedStyle(wrapper()).background)).toBe(ThemePalette.grayscale[8])
  })
