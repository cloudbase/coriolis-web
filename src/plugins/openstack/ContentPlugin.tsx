...
class ContentPlugin extends React.Component<Props, State> {
  ...
  get hasCephOptionsSet(): boolean {
    const cephOptionsField = this.props.connectionInfoSchema.find(n => n.name === 'ceph_options')
    if (!cephOptionsField || !cephOptionsField.properties) {
      return false
    }
    const hasValues = cephOptionsField.properties.filter(f => this.getFieldValue(f))
  ...
  getFieldValue(field?: Field | null) {
    ...
    const getInputChoiceValue = (fieldBaseName: string): string => {
      const id = this.props.getFieldValue(this.props.connectionInfoSchema.find(n => n.name === `${fieldBaseName}_id`))
      const previouslySelected = this.previouslySelectedChoices.find(f => f === `${fieldBaseName}_id`)
  ...
  findInvalidFields = () => {
    ...
    const cephOptions = this.props.connectionInfoSchema.find(f => f.name === 'ceph_options')
  ...
  filterSimpleAdvanced(): Field[] {
    ...
    const ignoreFields = ['user_domain_id', 'project_domain_id', 'user_domain_name', 'project_domain_name']
    ...
    return this.props.connectionInfoSchema
      .filter(f => !ignoreFields.find(i => i === f.name)).filter(field => {
        if (field.name === 'ceph_options') {
          return this.state.useAdvancedOptions
  ...
  renderFields() {
    ...
    const renderField = (field: any) => {
      ...
      const value = field.name === 'show_ceph_options' ? (this.state.showCephOptions || this.hasCephOptionsSet) : this.getFieldValue(field)
      ...
      return (
        <FieldStyled
          ...
          value={value}
          onChange={onChange}
          getFieldValue={fieldName => this.getFieldValue(
