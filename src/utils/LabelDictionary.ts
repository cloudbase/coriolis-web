...
class LabelDictionary {
   ...
   */
  static get(fieldName: string | null | undefined, dictionaryKey?: string): string {
    if (!fieldName) {
      return ''
    }
    const cachItem = cache.find(i => i.key === dictionaryKey && i.name === fieldName)
    ...
    words = words.map(word => {
      ...
      const newWord = acronym ? acronym.toUpperCase() : (abb[word] || word.toLowerCase())
