...
const projectsUrl = ({ resourceGroupName, ...other }: any) => `${resourceGroupsUrl({ ...other })}/${resourceGroupName}/providers/Microsoft.Migrate/assessmentprojects`
const groupsUrl = ({ projectName, ...other }: any) => `${projectsUrl({ ...other })}/${projectName}/groups`
const assessmentsUrl = ({ groupName, ...other }: any) => `${groupsUrl({ ...other })}/${groupName}/assessments`
const assessmentDetailsUrl = ({ assessmentName, ...other }: any) => `${assessmentsUrl({ ...other })}/${assessmentName}`
...
class Util {
  ...
  static sortAssessments(assessments: any[]) {
    assessments
      .sort((a: any, b: any) => moment(b.properties.updatedTimestamp)
        .toDate().getTime() - moment(a.properties.updatedTimestamp).toDate().getTime())
    return assessments
  }
...
class AzureSource {
  ...
  static getResourceGroups(subscriptionId: string): Promise<Assessment['group'][]> {
    return Api.get(Util.buildUrl(resourceGroupsUrl({ subscriptionId }), '2017-08-01')).then(response => Util.validateResponse(response, response.data.value))
  }
  ...
  static async getAssessments(subscriptionId: string, resourceGroupName: string, skipLog?: boolean | null): Promise<Assessment[]> {
    ...
    projects = projectsResponse.data.value.filter((p: any) => p.type === 'Microsoft.Migrate/assessmentprojects')

    // Load groups for each project
    const groupsResponses = await Promise.all(projects.map(async (project: any) => {
      let groups: any[] | null = null

      const groupsResponse = await Api.send({
        url: Util.buildUrl(
          groupsUrl({ projectName: project.name, subscriptionId, resourceGroupName }),
        ),
        cancelId,
      })
      ...
      groups = groupsResponse.data.value.map((group: any) => ({ ...group, project }))
      return groups
    }))
    ...
    return Promise.all(groups.map(group => Api.send({
      url: Util.buildUrl(assessmentsUrl({
        subscriptionId, resourceGroupName, projectName: group.project.name, groupName: group.name,
      })),
      cancelId,
  ...
  static async getAssessmentDetails(info: Assessment): Promise<Assessment> {
    const response = await Api.get(Util
      .buildUrl(
        assessmentDetailsUrl({ ...info, subscriptionId: info.connectionInfo.subscription_id }),
      ))
    const assessment: Assessment = await Util
      .validateResponse(response, { ...response.data, ...info })
  ...
  static getAssessedVms(info: Assessment): Promise<VmItem[]> {
    return Api.get(Util
      .buildUrl(assessedVmsUrl({ ...info, subscriptionId: info.connectionInfo.subscription_id })))
      .then(response => {
  ...
  static getVmSizes(info: Assessment): Promise<VmSize[]> {
    return Api.get(Util.buildUrl(`/subscriptions/${info.connectionInfo.subscription_id}/providers/Microsoft.Compute/locations/${info.location}/vmSizes`, '2017-12-01')).then(response => Util.validateResponse(response, response.data.value))
  }
}
